...
from .models import Weighting, Animal, RefBreed, RefAnimalType
...
class WeightingViewSetTestCase(APITestCase):
    def setUp(self):
        ...
        self.weighting_url = reverse('weighting-detail', kwargs={'pk': self.weighting.pk})
    ...
    def test_user_can_view_own_weightings(self):
        ...
        response = self.client.get(reverse('weighting-list'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), Weighting.objects.filter(user=self.user).count())
    
    def test_user_cannot_view_others_weightings(self):
        # Log in as a different regular user
        another_user = User.objects.create_user(username="another_user", password="another123")
    ...
    def test_admin_can_delete_any_weighting(self):
        ...
        self.assertFalse(Weighting.objects.filter(pk=self.weighting.pk).exists())
